...
def main(largeur_rec,hauteur_rec):
    ...
    if partie:
        ...
        nb_laby+=1
        histoire2=["Après avoir aisement franchis la première étape","L'aventurier se mis à chanter et crier de joie","Mais une fois la porte franchie","Son visage est devenu fermé et appeuré","Il venait de découvrir que son aventure au sein du Lawsbyrinthe","Venait seulement de commencer","Difficulté du niveau: taille augmentée"]
    ...
    if partie:
        ...
        nb_laby+=1
        histoire6=["Appeuré par les bêtes qui le poursuivait ","L'aventurier se mis à courir afin de fuir","Mais il sentait la chaleur monter et monter","Il fit alors une pause mais sentis rapidement de la fumée","Ainsi, il le compris ce n'était pas son corps","Le labyrinthe tout entier prennait feu ","Selon ses calculs dans 1m30 il ne sera plus qu'un tas de cendre","Difficulté du niveau : Feu/bête/clé"]
    ...
    if partie:
        ...
        nb_laby+=1
        histoire7=["Une fois de plus la mort ne s'emparra pas de l'aventurier","Pour autant la fumée engendrée s'est maintenant répandue","<<Annonce des gardiens du jeu>>","En raison de l'incendie certaine mise à jour sont apportées :","La difficulté affectée au niveau 7 est désormais annulée","De plus, une lampe torche est attribué à l'aventurier ","L'aventurier pris alors la lampe et commença le niveau","Difficulté du niveau : Fumée"]
...
def histoire_mort(largeur,hauteur,screen,nb_laby):
    ...
    pygame.display.update()
    time.sleep(2)
    part=["Après s'être vaillamment battu","Et avoir affronte " + str(nb_laby )+ " lawsbyrinthes",'Notre courageux aventurier a fini par échouer ','Et nous a desormais quitté','Le secret de Lawsbyrinthe restera alors protégé',"Pour l'instant, en attandant le prochain aventurier","Les gardiens ont une fois de plus réussi leur tâche  ",'En protégeant le lawsbyrinthe au péril de leur vie']
    taille_ecrit=0
        ...
        for event in pygame.event.get():
            if event.type ==pygame.QUIT: #cas ou on ferme la fenetre
                ...
                if pos[0]>=posxc1d and pos[0]<=posxc1f and pos[1]>=posyc1d and pos[1]<=posyc1f: # choix 1
                    ...
                    hist_c1_win=["Après avoir révélé au monde entier sa victoire ","Il fondit son entreprise de voyage luxueux","L'aventurier est désormais connu dans le monde entier ","Son entreprise, prospère est numéro 1 du marché ","Le Lawsbyrinthes perdit en revanche sa réputation de site à haut risque ","Et fut transformé en musée de commémoration aux aventuriers disparus"]
