...
{
    [TestClass]
    public class UnitTestCalculatrice 
    {
        ...
        public void TestOperateurs()
        {
            ...
            Assert.AreEqual("0+", calc.Operations);
            Assert.IsTrue(Math.Abs(calc.Resultat) < 1e-5, $"Le résultat {calc.Resultat} devrait être égal à 0");
            ...
            Assert.IsTrue(Math.Abs(calc.Resultat - 2) < 1e-5, $"Le résultat {calc.Resultat} devrait être égal à 2");
            ...
            Assert.IsTrue(Math.Abs(calc.Resultat + 4) < 1e-5, $"Le résultat {calc.Resultat} devrait être égal à -4");
            calc.Reset();
            // Test de la gestion des opérateurs
            calc.AddDigit(5);
            calc.AddDigit(4);
            Assert.AreEqual("54", calc.Operations);
            Assert.IsTrue(Math.Abs(calc.Resultat - 54) < 1e-5, $"Le résultat {calc.Resultat} devrait être égal à 54");
            calc.AddOperateur(Operation.ADDITIONNER);
            Assert.AreEqual("54+", calc.Operations);
            Assert.IsTrue(Math.Abs(calc.Resultat - 54) < 1e-5, $"Le résultat {calc.Resultat} devrait être égal à 54");
            calc.AddOperateur(Operation.SOUSTRAIRE);
