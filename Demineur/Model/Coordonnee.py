...
def construireCoordonnee(num_ligne,num_col):
    if type(num_ligne) != int or type(num_col) != int:
        raise TypeError(f"construireCoordonnee : Le numéro de ligne {type(num_ligne)} ou le numéro de colonne {type(num_ligne)} ne sont pas des entiers")
    elif num_ligne < 0 or num_col < 0 :
        raise ValueError(f"construireCoordonnee : Le numéro de ligne {num_ligne} ou de colonne {num_col} ne sont pas positifs")
    else:
        return (num_ligne,num_col)

def getLigneCoordonnee(coord):
    if not type_coordonnee(coord):
        raise TypeError("getLigneCoordonnee : Le paramètre n’est pas une coordonnée")
    return coord[0]
def getColonneCoordonnee(coord):
    if not type_coordonnee(coord):
        raise TypeError("getLigneCoordonnee : Le paramètre n’est pas une coordonnée")
    return coord[1]
